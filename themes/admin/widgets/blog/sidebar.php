<div class="dash_content_sidebar">
    ...
    <nav>
        ...
        $nav = function ($icon, $href, $title) use ($app) {
            ...
            $url = url("/admin/{$href}");
            return "<a class=\"icon-{$icon} rounded {$active}\" href=\"{$url}\">{$title}</a>";
        };
        ...
        <?php if (!empty($post->cover)): ?>
            <img class="rounded" style="width: 100%; margin-top: 30px" src="<?= image($post->cover, 680); ?>"/>
        <?php endif; ?>

        <?php if (!empty($category->cover)): ?>
            <img class="rounded" style="width: 100%; margin-top: 30px" src="<?= image($category->cover, 680); ?>"/>
        <?php endif; ?>
    </nav>
</div>